...
const CoffeeCard = ({
  ...
  return (
    <div className="mx-auto mt-5 md:mt-10 w-full max-w-60">
      <div className="relative">
        <img className="rounded-xl w-full h-40 object-cover" src={image} alt="Coffee" />
      ...
      <div className="flex items-center justify-between mt-1 sm:mt-2 ">
        {rating ? (
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="ml-1 text-sm text-[#6F757C] leading-none">No ratings</span>
          </div>
        )}

        {available ? null : (
          <span className="text-[#ED735D] text-sm  font-bold mt-1">Sold out</span>
        )}
      </div>
    </div>
  );
